...
% Smoothing can also be done using MATLAB's built-in function 'movmean'. 
% With 'movmean', we don't even need to loop through the columns!:
%Data_smooth_b = movmean(Data,framespan2smooth);

% Let's choose the movmean version:
%Data_smooth = Data_smooth_b;
...
cop_x_mm_zeromean = 1000*(cop_x-mean(cop_x));
cop_y_mm_zeromean = 1000*(cop_y-mean(cop_y));
...
cop_pathlength = COP_pathlength(cop_x,cop_y);
...
% create time vector
time = [1:length(cop_x)]/sf;

% fit smoothing spline through COP data
ftype =  fittype('smoothingspline');
x_fit = fit(time, cop_x, ftype);
y_fit = fit(time, cop_y, ftype);

% differentiate
x_velocity=differentiate(x_fit,time);
y_velocity=differentiate(y_fit,time);

%% Calculate COP speed using Pythagoras' theorem
...
function [ COP_path_length ] = COP_pathlength( cop_x , cop_y )
%COP_PATHLENGTH Calculates length, in meters, of COP path.
%   Inputs: cop_x = time series of x-coordinates of COP, in meters.
%           cop_y = time series of y-coordinates of COP, in meters.
%   Output: Length of 2D COP path, in meters.

COP_path_length = sum(sqrt(diff(cop_x).^2+diff(cop_y).^2));
