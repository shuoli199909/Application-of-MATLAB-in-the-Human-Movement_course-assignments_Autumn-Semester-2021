...
% -- my solution:
clear; close all; clc
file_loc = "walking_preferred.txt";
file_to_scan = fopen(file_loc);
num_head = 5;
delimiter_data = "\t";
data = textscan(file_to_scan, "%f %f %*[^\n]", "headerlines", num_head, "delimiter", delimiter_data);
eval(['time','=data{1,1};']);
eval(['angle','=data{1,2};']);
figure(1);
plot(time, angle);
...
swing_pointer(swing_pointer==0) = 20*(i)+10;
...
%swing_max_all(1) = max(angle(swing_pointer==2)); 
% Do you see the logical indexing I used?
...
% -- my solution:
for i = 1:7
   swing_max_all(i) =  max(angle(swing_pointer==2*i));
end
% -- end
